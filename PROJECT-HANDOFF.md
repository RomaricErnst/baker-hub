...
## Explicit preferment lock and touch interaction — 26 September 2026

User supplied a115s physical-iPhone recording showing Safari chrome movement before a second tap, Vercel toolbar interference near Modifier, and preferment15:00 staying fixed after mixing changes. Authorized all proposed fixes, preview only. Previous final CI36116727097 is now verified SUCCESS:310 unit tests/build and204/204 WebKit cases. Those tests did not establish native Safari chrome behavior.

Current changes: remove After baking from planning (guide cooling instructions remain); offer alternative bake presets only before choosing or when the current candidate is invalid. Commercial preferment follows the existing getPrefOptH recommendation when mixing is edited unless explicitly locked. New persisted prefLocked flag requires a valid pref timestamp; legacy edited preferment timestamps remain saved but are released on a subsequent mixing edit unless explicitly locked. Checkbox exposed in its editor and remains visible when locked. Apply/Cancel remain transactional, full-candidate validation still guards commits; reset is above key actions.

Native range drags update local time immediately, suspend slot probing/valid markings during the gesture and solve on release. Keyboard/exact input still apply drafts immediately. Slot probes yield per candidate and restart after120ms. Touch footer lifted24px from browser edge; header auto-hide is held while a control pointer is down and touch header animation removed. Vercel toolbar host hidden only when VERCEL_ENV=preview; dashboard settings required login, so injection setting was not changed. Native Safari absorption of first touch cannot be proven fixed by emulator.

Local311 tests passed and TypeScript passed during development. New browser regression covers automatic following, explicit lock/unlock, persistence and reset placement. Final CI/deployment/live verification pending; do not claim these new cases passed yet. No biological coefficients changed.

Follow-up preview app `ac7811efbe338d36b67ea3635bd93facf676d988` (local62438d5) is READY at deployment `dpl_EBFesarpsAkfUKBNBgjXSySfzFJU`. First runs were cancelled by corrective pushes after failures: WebKit range did not move with explicit pointer capture, lifted footer obscured the fillings-card center at320px, calendar expectation retained an obsolete12h rather than existing15h optimum, and unlock fixture was constrained by earliest future preparation. Removed range pointer capture (window pointerup/cancel provides cleanup), added footer spacing/scroll margin, and moved the lock scenario to future morning while retaining strict assertions. Live final preview real drag moved mixing07:00→08:45 and preferment18:00→19:45 at unchanged19:30 bake. Saved lock survived reload; reset removed lock and returned18:00/07:00 while retaining nights. Current CI36206213167 pending completion; do not report success until verified.

Final evidence: CI36206213167 passed311 unit tests/build and200/208 browser cases, including every scheduling, drag, lock/unlock/reset and persistence check at all four widths. Its eight failures were the same fillings-card hit-target check in both modes across four widths: scrollIntoViewIfNeeded left its center beneath the lifted sticky action bar. Test-only commit `1b126dd1f9297341133602d0211e8981482accdd` centers the card as a user can by scrolling and retains the strict elementFromPoint plus single-tap assertions. Final CI [36206997524](https://github.com/RomaricErnst/baker-hub/actions/runs/36206997524) is SUCCESS:311/311 unit tests, build and208/208 WebKit cases. Application is unchanged from `ac7811e`; preview deployment `dpl_Eq6pMeRdHN16TfjMZt3x8zkJo73X` is READY. Actual320/390px horizontal-editor screenshots were inspected; controls fit, and the390px completed result shows the green compatible band. The320px capture occurred during explicitly labeled slot calculation. Live checks establish automatic following, explicit lock/reload and reset, not native Safari chrome absorption. Production was not promoted.

## Linked scheduling editor and guided repair — 26 September 2026

User authorized all recommended UX improvements after a second139s iPhone recording, explicitly asking to open both sliders together. Application/test commit `b5f71b2a0c86ddb51ca145228c5c66a7f13513e7` (local47fb724) is deployed READY as `dpl_4BBvVnY3xtuC6Q7VpfSjkrYPPJP9` on the existing preview branch. No production promotion.

- Either Modify opens one shared panel with both horizontal controls, related-time recap, retained oven target and one Apply/Cancel. Automatic preferment following and explicit locking are retained. Draft consequence text identifies the moved dependent action; reset remains above the plan.
- Conflict notes follow the responsible action, rather than the last edited row. Recommendation/custom/draft/conflict/drag states are distinct. Alternative oven times use canonical whole-plan search with recurring availability and explicit locks, and commit the validated plan atomically; obsolete unlocked preferment metadata is released.
- Video case diagnosis: under the stated Neapolitan/cold-poolish fixture, Saturday noon + Sunday11:30 bake + nights blocked yields no supported fixed-bake result in the bounded model. Sunday19:30 does. The fix is a verified alternative and explanatory path, not relaxed fermentation bounds or a claim that the solver never ran.
- Mobile setup header no longer hides/reveals on scroll. Step actions and coarse-pointer batch actions are in normal document flow, removing the old lifted sticky bar that could cover controls. Step changes blur stale focus, reset before paint and stop delayed scroll settling upon user interaction. Clock display faces use24h while preserving native pickers. Availability presets have clear unselected contrast and aria-pressed.

Local311 unit tests and TypeScript passed. CI36217830134 completed201/216 WebKit width cases successfully. Fifteen failures: seven navigation controls were scrolled out of view again by the new250ms reset; four new coupling checks incorrectly expected preferment to leave its earliest future slot; four new alternative checks searched19:30 instead of the actual French19h30. Artifact10898735222 confirms the proposed Sunday19h30 and the valid unchanged9h preferment/22h mix. Removed delayed reset and stop frame settling after a nonzero scroll; retained strict hit tests. Coupling regression now checks both a legitimately retained preferment and a future-morning edit that must move it. Feedback says “conservé” when unchanged instead of falsely “recalé”. Final rerun pending.

Vercel deployment build is successful. Live preview UI verification is blocked this turn: automatic browser approval denied the redirect to generic Vercel login as outside the scoped preview and potentially exposing unrelated account content. Do not circumvent it; inspect CI artifacts instead. Native physical-iPhone first-tap/browser-chrome behavior remains unproven.

Final application/test commit `efe807280d8bd3c857830edb58366936c2452e3c` (localaf1d520): CI [36225435962](https://github.com/RomaricErnst/baker-hub/actions/runs/36225435962) is SUCCESS,311 unit tests/build and216/216 WebKit cases across320/375/390/430px. All15 prior failures are resolved with the strict navigation hit checks unchanged. Both simultaneous editors, retained-versus-moved preferment, Cancel, explicit lock/reset, and applying the verified Sunday19h30 alternative with nights retained pass. Artifact10900198247 contains the final reports and screenshots. Deployment `dpl_DVuc3Cm4AHgP9ksjv3DdiWPttFft` is READY and assigned to the stable preview alias. Production unchanged; native Safari browser-chrome absorption remains a separate physical-device validation limit.

Final narrow and wider linked-editor captures reviewed: both green ranges, the linked16:15 preferment/07:15 mixing recap, retained19:30 oven target, lock, exact24h entries and common Apply/Cancel are readable. This is a scrollable shared panel, not a promise that both full sliders fit simultaneously in a single phone viewport. Scoped screenshots include a sticky-header overlay at their top; automated single-tap/header/scroll checks pass separately. No physical-device confirmation is implied.

## Compact chronological scheduling — 26 September 2026

User approved the inline vertical chronology and explicitly authorized deploying the recommended prototype. It replaces the shared horizontal-slider panel, preserving its real solver and transactional edits. The neutral spine expresses event order, not elapsed duration. Days group once; one selected time editor replaces that row's displayed time, with exact date/time and ±30 minute controls. Related event times remain visible; compatible windows are computed only for the selected event. Apply/Cancel appear only for changed drafts. Existing fixed oven target, availability constraints, guided corrections and return to recommendation remain.

Following independent UX reviews, the explicit keep-time choice moves into the mixing editor, with a brief explanation of automatic preparation recalculation. Preparation rows show “Heure conservée” only for explicit choices. Poolish/biga and future sourdough pre-mix feeds use the same interaction; new feedLocked metadata persists the latter choice. Historical starter actions and calculated transfers remain read-only. No biological coefficients changed. Local verification and preview CI/deployment results are pending at this checkpoint. Native Safari browser-chrome first-touch absorption still requires physical-device confirmation; use the authorized CI workflow/artifacts, not the previously blocked generic Vercel login route.

Compact timeline app/test commit ecdef3bb6883d9aba34bab6407d676244c3f3993 (local3010f81) is deployed READY as dpl_6SpopxgnZKjBbpJYxEYE2h3gek7W on the stable preview alias. CI36232980883 passed314 unit tests/build and216/224 mobile cases. The eight failures are the same adapted sourdough exact-entry assertion across RT/fridge and four widths: expected raw candidate seconds/milliseconds although datetime-local entry deliberately supplies minute precision. Correct the test expectation to the entered local minute without relaxing equality. New keep-only sourdough Apply, lock/reload/release/reset, historical immutability, compact editor/step taps, availability and poolish coupling passed at all four widths. Final rerun pending; production untouched.

Final compact-timeline verification: the corrected test-only commit b3d9c3bf61b8f0e058bc2b42a31bcb0922e60f21 (local76b543b) passes the iPhone interaction step in CI36233696870, with unit tests and build also successful. Preview dpl_7LZBxbAH4odey5kFcUcxAXbRsbPm is READY; application code is unchanged from ecdef3b. Reviewed actual320/390px poolish timeline captures and390px sourdough final-feed capture from artifact10903311647: day grouping, one active exact editor, ±30 controls, linked times, Apply/Cancel, and green sourdough compatible-window text are readable. Scoped poolish captures include the sticky header at the top and show the explicitly labeled window-calculation state immediately after switching rows; separate assertions wait for final calculations. Full physical Safari chrome behavior remains unverified.

CI36233696870 is now complete SUCCESS:314/314 unit tests, build and224/224 WebKit scenarios across320/375/390/430px. Final report artifact10903227648. All eight exact-entry precision assertions pass after matching the entered minute; no application change was required. Deployment and final validation are complete.

## Discreet selected-style edit — 26 September 2026

User approved replacing the oversized quantity-page “Changer de pizza/pain” button with a small inline “Modifier” (“Edit”) beside the selected style/bread. The control has no filled surface/border, retains a44px touch target and contextual accessible name, and wraps safely on narrow layouts. Its existing return-to-style handler and retained choices are unchanged. Adapted the two existing bread-return browser selectors to the new accessible name. TypeScript and test syntax checks passed; preview build verification follows publication. This cosmetic change does not claim a fresh224-case browser result.

## Page-arrival scroll coordination — 26 September 2026

User reports the mode-choice heading still clipped under Organisation on physical Safari and asks for a navigation-wide fix. Common page-key resets already exist, but finish after two frames; keyboard/toolbar viewport changes can arrive later. New shared usePageTop handles committed page keys, browser pageshow and late viewport resize until the first new deliberate interaction. It preserves manual scroll and avoids delayed timer-based jumps. Applied to both new-bake and existing-base journeys. Removed competing delayed recipe-results scrolls and SandwichParty's pre-commit scroll when parent navigation owns the transition. Header focus restoration uses preventScroll; header height is measured before paint; navigator top no longer animates, and browser anchoring is disabled across the shared page root. Navigation tests now assert top arrival in the all-sections helper, plus a targeted delayed-viewport/BFCache case that checks the heading's top edge and deliberate-scroll preservation. This simulates viewport events, not native Safari chrome; physical-device confirmation remains necessary. Preview verification pending.

Initial arrival CI36235811878 passed314 unit tests/build and222/228 WebKit cases. Six320px failures exposed a competing BakeGuide activation scroll to the remembered instruction (scrollY325, hardcoded120px header). Restrict guide scrolling to explicit next/previous actions within the same phase; section arrival now retains page top, while internal movement uses actual header bounds. Pageshow also blurs stale focus. Local TypeScript and314 tests passed after this correction; final preview CI pending.

User's latest visual feedback: align Organisation and the step control on a shared text baseline, underline the step label and use a drawn chevron while retaining44px touch height. Remove the estimated preferment gram sentence; retain the suggested flour percentage and rationale disclosure. Replace the schedule's platform-dependent arrow emoji with a discreet underlined Modifier/Edit inside the existing contextual time button. These changes are included in the same preview update; production remains untouched.

Latest visual/navigation application commit63b168d348dddac3138965597fe002fdd585e4b5 is deployed READY. CI36237241733 stopped on the pre-existing preferment guidance assertion requiring gram quantities, which the user explicitly requested removing. Updated that assertion to require no estimated gram sentence while retaining bilingual rationale checks; local314/314 tests pass. Test-only commit266d2356a7a40c6ead4a4f0e77788b675a4a14e9 is deployed READY as dpl_8nhnzP5HVhd7ABs81C7UMgLgfsEA; full mobile run36237412748 is still pending. Do not claim final228-case success before checking this run.

New user feedback (discussion, not yet implemented): Simple wrap journey still requires too many clicks; group its easier protocol, provide ingredient substitutions, add Mexican-inspired filling options. Source review: simple setup has distinct equipment/climate/yeast/planning steps; piadina omits yeast but still traverses kitchen/setup. Current laffa is yeasted and oven-cooked, piadina is unleavened and pan-cooked: do not silently turn laffa into an unfermented tortilla. Recommended direction shared: quantity+filling then ready recipe, defaults editable in place, one grouped preparation/rest/cooking/filling protocol. Structured substitutions must update ingredients, shopping and instructions, not merely append a note. Proposed Mexican-inspired concepts: chicken/peppers/lime; black beans/corn/avocado; mushroom/peppers/cheese. No new dough formula, substitutions or recipes have been deployed.

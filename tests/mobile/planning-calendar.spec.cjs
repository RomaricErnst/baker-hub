...
async function openPlan(page,mode='simple',blocks=[]){
 const bake=new Date();bake.setDate(bake.getDate()+5);bake.setHours(18,0,0,0);
 const start=new Date(+bake-26*3600000);
 const data={version:1,savedAt:Date.now(),tab:mode,bakeType:'pizza',styleKey:'neapolitan',numItems:2,itemWeight:250,pizzaDiameter:30,ovenType:'pizza_oven',mixerType:'hand',yeastType:'instant',kitchenTemp:22,humidity:'normal',fridgeTemp:5,flourBlend:null,prefermentType:'none',prefOffsetH:0,prefGoesInFridge:true,flourInFridge:false,startTime:+start,eatTime:+bake,blocks,recipeGenerated:true,modeChosen:true,qtyChosen:true,flourChosen:true,prefermentChosen:true,activeStep:7,advancedStep:9,highestStep:7,advancedHighestStep:9,setupOverview:false,activeTab:'setup'};
 await page.addInitScript(data=>{localStorage.setItem('bh_session_v1',JSON.stringify(data));sessionStorage.setItem('bh_locale_resume',JSON.stringify({activeStep:7,advancedStep:9,setupOverview:false,activeTab:'setup',reviewMode:true}));},data);
...
for(const mode of ['simple','custom'])test(`${mode}: quiet calendar, draft cancel and checked commit`,async({page},testInfo)=>{
 ...
 await expect(plan.getByText(/intervalle raccourci/).first()).toBeVisible();
 await expect(plan.getByText('Préchauffage',{exact:true})).toBeVisible();
 await plan.getByRole('button',{name:'Modifier les horaires',exact:true}).tap();
 await plan.getByRole('button',{name:'Modifier Commencer la pâte',exact:true}).tap();
 const editor=plan.locator('input[type="datetime-local"]');await expect(editor).toBeVisible();
 ...
 await expect(plan.getByText('Proposition non enregistrée').first()).toBeVisible();
 ...
 await plan.getByRole('button',{name:'Modifier Commencer la pâte',exact:true}).tap();
 await editor.fill(local(new Date(+bake-60*60000)));await expect(plan.getByRole('button',{name:'Garder cet horaire',exact:true})).toBeDisabled();
 await expect(plan.getByText('Préchauffage',{exact:true}).locator('..').locator('..').getByText('45 min',{exact:true})).toBeVisible();
 await editor.fill(local(proposed));await expect(plan.getByRole('button',{name:'Garder cet horaire',exact:true})).toBeEnabled();
 await plan.getByRole('button',{name:'Garder cet horaire',exact:true}).tap();
 await expect.poll(async()=>(await stored(page)).startTime).toBe(+proposed);expect((await stored(page)).eatTime).toBe(+bake);
 await expect(plan.getByText(/Horaire conservé/)).toBeVisible();

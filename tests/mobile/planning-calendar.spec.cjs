...
test('levain known peak uses maturity window; observation never becomes a draggable feed',async({page})=>{
 ...
 await expand(plan,'mix');await expect(row(plan,'mix').getByRole('slider')).toBeVisible();await expect(plan.locator('[data-key-timing^="starter:"] input[type="range"]')).toHaveCount(0);
 ...
 await page.reload();await expect(plan).toBeVisible();await expand(plan,'mix');await expect(row(plan,'mix').getByRole('slider')).toHaveValue(String(+start+900000));
});
for(const storage of ['rt','fridge'])test(`levain ${storage} future feeds use starter solver and cancel leaves saved plan intact`,async({page})=>{
 ...
 await expand(plan,'mix');const mix=row(plan,'mix').getByRole('slider');await expect(mix).toBeVisible();const original=(await stored(page)).startTime;
 await mix.focus();await mix.press('ArrowRight');await expect(plan.locator('[data-key-timing^="starter:"]')).not.toHaveCount(0);
 ...
 await expand(plan,'mix');await mix.focus();await mix.press('ArrowRight');await expect(confirm(plan)).toBeEnabled();await confirm(plan).click();
 ...
 await expand(plan,'mix');await expand(plan,'starter:pre_mix');const feed=row(plan,'starter:pre_mix').getByRole('slider');await expect(feed).toBeVisible();const limits=[await feed.getAttribute('min'),await feed.getAttribute('max')];await feed.focus();await feed.press('ArrowRight');await expect(feed).toHaveAttribute('min',limits[0]);await expect(feed).toHaveAttribute('max',limits[1]);await expect(mix).toHaveValue(String(original+900000));await plan.getByRole('button',{name:'Annuler',exact:true}).click();
});

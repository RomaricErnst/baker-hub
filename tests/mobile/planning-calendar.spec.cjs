...
async function openPlan(page,mode='simple',blocks=[],preferment=false,night=false){
...
test('vertical slider keeps morning mixing through a blocked night and supports undo',async({page},testInfo)=>{
 ...
 await expect.poll(async()=>(await stored(page)).prefOffsetH).not.toBe(12);
 expect((await stored(page)).startTime).toBe(+start);expect((await stored(page)).eatTime).toBe(+bake);
 await page.getByRole('button',{name:'Annuler le dernier ajustement',exact:true}).tap();
 await expect.poll(async()=>(await stored(page)).prefOffsetH).toBe(12);
});
...
for(const mode of ['simple','custom'])test(`${mode}: preferment window preserves mixing, applies and survives reload`,async({page})=>{
 ...
 await expect(plan.getByRole('button',{name:'Appliquer',exact:true})).toBeEnabled();
 await plan.getByRole('button',{name:'Appliquer',exact:true}).tap();
 await expect.poll(async()=>(await stored(page)).prefOffsetH).toBe(11.75);
 expect((await stored(page)).startTime).toBe(+start);expect((await stored(page)).eatTime).toBe(+bake);

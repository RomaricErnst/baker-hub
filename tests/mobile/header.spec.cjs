...
for (const locale of ['fr', 'en']) {
  test(`${locale}: iPhone header, touch menu, local save and language`, async ({ page }, testInfo) => {
    ...
    await expect(page.getByRole('heading', { name: navV2 ? (fr ? 'Choisissez votre pizza' : 'Choose your pizza') : (fr ? 'À votre façon' : 'Your way'), exact: true })).toBeVisible();
    ...
    await expect(page.getByRole('heading', { name: navV2 ? (fr ? 'Choose your pizza' : 'Choisissez votre pizza') : (fr ? 'Your way' : 'À votre façon'), exact: true })).toBeVisible();
    await expect(dialog).toBeHidden();

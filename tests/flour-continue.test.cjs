...
function run(name,chosen,archived=[]){
 const calls=[];
 const context={styleKey:'neapolitan',commercialPrefermentPlanReady:true,advancedRecipe:null,recipe:null,tab:'custom',flourChosen:chosen,archivedFlourNames:archived,CUSTOM_STEPS:[],advancedHighestStep:6,suppressNextScrollRef:{current:false},nextUnanswered:()=>7,scrollToStepTop:()=>calls.push(['scroll'])};

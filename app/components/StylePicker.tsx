...
export default function StylePicker({bakeType,selected,onSelect,disabledIds=[],disabledNote}:StylePickerProps){
 ...
 function card(key:string){
  ...
  return <div key={key}><button className={`dough-style-card${bread?' bh-bread-style-card':''}`} type="button" disabled={disabled} aria-pressed={active} onClick={()=>onSelect(key as StyleKey)}
   ...
   <span style={{minWidth:0}}><strong style={{display:'block',fontSize:16}}>{title}</strong>{active&&<span style={{display:'block',color:'var(--terra)',fontSize:12,fontWeight:600,marginTop:4}}><span aria-hidden="true">✓ </span>{fr?'Sélectionné':'Selected'}</span>}<span style={{display:'block',fontSize:13,lineHeight:1.5,color:'var(--ash)',marginTop:4}}>{description}</span>{meal?<span style={{display:'flex',alignItems:'center',gap:8,marginTop:7,fontSize:12,fontWeight:600,color:'var(--terra)'}}><Image src={`/images/approved/sandwich/${meal.recipeId}.webp`} alt="" width={640} height={480} sizes="44px" style={{width:44,height:33,objectFit:'cover',borderRadius:5,flexShrink:0}}/><span>{fr?(meal.kind==='tartine'?'Idée tartine':meal.kind==='wrap'?'Idée wrap':'Idée sandwich'):(meal.kind==='tartine'?'Tartine idea':meal.kind==='wrap'?'Wrap idea':'Sandwich idea')}</span></span>:use&&<span style={{display:'block',fontSize:12,fontWeight:600,color:'var(--terra)',marginTop:6}}>{use}</span>}</span>

 ...
 function card(key:string){
  ...
  return <div key={key}><button className={`dough-style-card${bread?' bh-bread-style-card':''}`} type="button" aria-label={title} disabled={disabled} aria-pressed={active} onClick={()=>onSelect(key as StyleKey)}
   ...
   {bread?<span className={`bh-bread-photos${meal?' bh-bread-photos-pair':''}`}>
    <span className="bh-bread-photo"><Image src={s.image} alt="" fill sizes="(max-width:700px) 50vw, 220px" style={{objectFit:'contain'}}/><span className="bh-bread-photo-caption">{fr?'Le pain':'The bread'}</span></span>
    {meal&&<span className="bh-bread-photo"><Image src={`/images/approved/sandwich/${meal.recipeId}.webp`} alt="" fill sizes="(max-width:700px) 50vw, 220px" style={{objectFit:'cover'}}/><span className="bh-bread-photo-caption">{fr?(meal.kind==='tartine'?'Idée tartine':meal.kind==='wrap'?'Idée wrap':'Idée sandwich'):(meal.kind==='tartine'?'Tartine idea':meal.kind==='wrap'?'Wrap idea':'Sandwich idea')}</span></span>}

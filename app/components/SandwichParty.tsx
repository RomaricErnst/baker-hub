...
export default function SandwichParty({isFr,styleKey,snapshot,onChange,breadIngredients=[],availableDoughWeight,numItems,onAdjustBread,hideNavigation=false,doughConfigured=true}:SandwichPartyProps) {
  ...
  return <div className={styles.party}>
    <div className={styles.hero}>
      ...
      {family && <img src={family.image} alt={tr(family.name)}/>}
    </div>
    {(!family || choosingFamily) && <div className={styles.card}>
      <h3>{t('Quel pain allez-vous garnir ?','Which bread will you fill?')}</h3>
      <p className={styles.muted}>{t('Ce choix concerne les sandwichs ; il ne change pas votre recette de pain.','This choice is for sandwiches; it does not change your bread recipe.')}</p>
      <div className={styles.filters}>{SANDWICH_FAMILIES.map(item=><button type="button" key={item.id} className={styles.button} aria-pressed={family?.id===item.id} onClick={()=>chooseFamily(item)}>{tr(item.name)}</button>)}</div>
    </div>}
    {family && <>
    ...
    {detail&&family&&<div className={styles.backdrop} onClick={event=>{if(event.target===event.currentTarget)setDetailId(null);setReviewOpen(false);}}>
      <div className={styles.sheet} role="dialog" aria-modal="true" aria-labelledby={detailHeading} ref={dialogRef}>
        ...
        <div className={styles.sheetBody}>
          ...
          <p className={styles.muted}>{t('Pour un sandwich','For one sandwich')} · ≈ {kcal(detail)} kcal · {t('pain inclus','bread included')}</p>
          <h3>{t('Pain','Bread')}</h3><p>{tr(family.name)} · ≈ {detail.breadGrams} g</p>
          ...
          <h3>{t('Garnitures par sandwich','Fillings per sandwich')}</h3>

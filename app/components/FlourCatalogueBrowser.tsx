...
export default function FlourCatalogueBrowser({onChoose,recommendedIds,onGeneric,onScan}:{onChoose:(f:FlourEntry)=>void;recommendedIds:string[];onGeneric:()=>void;onScan:()=>void}){
 ...
 return <div ref={top}>
 <label style={{display:'block',marginBottom:8}}>{fr?'Marque, farine ou type':'Brand, flour or type'}<input type="search" value={query} onChange={e=>{setQuery(e.target.value);setPage(0);}} onKeyDown={e=>{if(e.key==='Enter')e.currentTarget.blur();}} placeholder={fr?'Ex. Caputo, T65, complète':'e.g. Caputo, T65, wholemeal'} style={{...field,marginTop:6}} aria-label={fr?'Rechercher une farine':'Search flour'}/></label>
 <details style={{margin:'8px 0'}}><summary style={{minHeight:44,padding:'10px 0',cursor:'pointer',fontSize:16}}>{fr?'Filtrer par marque, type ou pays':'Filter by brand, type or country'}</summary><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:10}}>{[{label:fr?'Marque':'Brand',value:brand,set:setBrand,values:catalog.map(f=>brandOption(f.brand))},{label:'Type',value:type,set:setType,values:catalog.map(f=>flourTypeOption(f.type))},{label:fr?'Origine':'Origin',value:country,set:setCountry,values:catalog.map(f=>f.country)}].map(d=><label key={d.label} style={{fontSize:14}}>{d.label}<select value={d.value} style={field} onChange={e=>{d.set(e.target.value);setPage(0);}}><option value="">{fr?'Tous':'All'}</option>{[...new Set(d.values)].sort().map(v=><option key={v} value={v}>{d.label===(fr?'Origine':'Origin')?new Intl.DisplayNames([fr?'fr':'en'],{type:'region'}).of(v.toUpperCase())||v:d.label==='Type'?flourTypeLabel(v,fr):v}</option>)}</select></label>)}</div></details>
 ...
 <div style={{display:'flex',flexWrap:'wrap',gap:10,margin:'12px 0'}}><button type="button" onClick={onGeneric} style={{minHeight:44,padding:'10px 12px',border:'1px solid var(--border)',borderRadius:9,fontSize:14}}>{fr?'Utiliser un type de farine':'Use a flour type'}</button><button type="button" onClick={onScan} style={{minHeight:44,padding:'10px 12px',border:'1px solid var(--border)',borderRadius:9,fontSize:14}}>{fr?'Scanner le sac':'Scan bag'}</button></div>
 <h3 style={{fontSize:16,fontWeight:600,margin:'20px 0 10px'}}>{initial?(fr?'Pour votre style':'For your style'):`${filtered.length} ${fr?'résultat(s)':'results'}`}</h3>

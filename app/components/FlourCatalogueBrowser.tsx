...
export default function FlourCatalogueBrowser({onChoose,recommendedIds,onGeneric,onScan,styleKey}:{onChoose:(f:FlourEntry)=>void;recommendedIds:string[];onGeneric:()=>void;onScan:()=>void;styleKey?:string}){
 ...
 <h3 style={{fontSize:16,fontWeight:600,margin:'20px 0 10px'}}>{active?`${filtered.length} ${fr?'résultats':'results'}`:(limit>4?(fr?'Toutes les farines · choix populaires en premier':'All flours · popular choices first'):(fr?`Choix populaires${styleLabel?' · '+styleLabel:' pour ce style'}`:`Popular choices${styleLabel?' · '+styleLabel:' for this style'}`))}</h3>

...
export default function ScheduleTimeSlider({from,to,value,onChange,blocks,isFr,label,slots=[]}: {
  ...
  return <div style={{display:'grid',gridTemplateColumns:'48px minmax(0,1fr)',gap:12,minHeight:220}}>
    <div style={{position:'relative',height:220,background:'#e4e1db',borderRadius:24}}>
      ...
      <input type="range" aria-label={label} aria-valuetext={fmt(value)} min={from} max={to} step={15*60000} value={Math.max(from,Math.min(to,value))} onChange={e=>onChange(Number(e.target.value))} onKeyDown={event=>{if(event.key==='ArrowDown'||event.key==='ArrowUp'){event.preventDefault();onChange(Math.max(from,Math.min(to,value+(event.key==='ArrowDown'?900000:-900000))));}}}

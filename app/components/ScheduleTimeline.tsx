...
export default function ScheduleTimeline({rows,blocks,isFr,editingEnabled,editingId,onEdit,editor,onToggleEditing}: {
  ...
  return <section aria-label={isFr?'Votre planning':'Your schedule'} style={{marginTop:20}}>
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,flexWrap:'wrap',marginBottom:12}}>
      ...
      <button type="button" aria-expanded={editingEnabled} style={button} onClick={onToggleEditing}>{editingEnabled?(isFr?'Terminer les modifications':'Finish editing'):(isFr?'Modifier les horaires':'Edit times')}</button>
    </div>
    <p style={{fontSize:13,color:'var(--smoke)',margin:'0 0 14px'}}>{isFr?'Les longues attentes sont raccourcies à l’écran. Les heures restent exactes.':'Long waits are compressed on screen. The times remain exact.'}</p>
    {items.map((item,index)=>{
      ...
      return <div key={r.id}>{day}<div style={{display:'grid',gridTemplateColumns:'54px minmax(0,1fr)',gap:12,padding:'10px 0',alignItems:'start'}}>
        <time dateTime={new Date(r.at).toISOString()} style={{fontSize:15,fontVariantNumeric:'tabular-nums',paddingTop:10}}>{r.originalAt!==undefined&&<span style={{display:'block',textDecoration:'line-through',opacity:.5,fontSize:12}}>{fmt(r.originalAt)}</span>}{r.timeText.startsWith('≈')?'≈':fmt(r.at)}</time>
        <div style={{borderLeft:`3px solid ${r.color}`,padding:'10px 12px',borderRadius:5,background:'var(--warm)',minWidth:0,opacity:r.isHistory?.7:1}}>
          <div style={{display:'flex',gap:8,justifyContent:'space-between',alignItems:'center',flexWrap:'wrap'}}><strong style={{fontSize:16}}>{r.name}</strong>{editingEnabled&&r.editable&&editingId!==r.id&&<button type="button" style={button} aria-label={`${isFr?'Modifier':'Edit'} ${r.name}`} onClick={()=>onEdit(r.id)}>{isFr?'Modifier':'Edit'}</button>}</div>
          ...
          {r.originalAt!==undefined&&<div style={{fontSize:13,color:'var(--terra)'}}>{isFr?'Proposition non enregistrée':'Unsaved proposal'}</div>}{editingId===r.id&&<div style={{marginTop:10}}>{editor}</div>}
          {r.note&&<div style={{marginTop:8,fontSize:14,lineHeight:1.5}}>{r.note}</div>}
        </div>
      </div>{wait>=15*60000&&<div style={{margin:'0 0 8px 66px',minHeight:Math.min(100,20+Math.sqrt(wait/3600000)*14),display:'flex',alignItems:'center',flexWrap:'wrap',gap:4,padding:'8px 12px',borderLeft:'2px dotted var(--border)',fontSize:14,color:'var(--smoke)'}}>{r.waitLabel??(isFr?'Attente':'Wait')} · {timelineDuration(wait)}{wait>=2*3600000&&<span> · {isFr?'intervalle raccourci':'compressed interval'}</span>}</div>}</div>;
    })}
  </section>;
}

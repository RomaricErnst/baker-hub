...
export default function ScheduleTimeline({rows,blocks,isFr,editingEnabled,editingId,onEdit,editor,onToggleEditing}: {
  ...
  return <section aria-label={isFr?'Étapes du planning':'Schedule steps'} style={{marginTop:20}}>
    ...
    {items.map((item,index)=>{
      ...
      return <div key={r.id} data-schedule-step={r.id} data-at={r.at}>{day}<div style={{display:'grid',gridTemplateColumns:'54px minmax(0,1fr)',gap:12,padding:'10px 0',alignItems:'start'}}>

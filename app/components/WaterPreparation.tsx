...
interface Props extends WaterSettingsProps {
  ...
  idealWaterTemp?: number; targetDoughTemp?: number; directIceSupported?: boolean;
}
...
export default function WaterPreparation({waterGrams,targetTemp,kitchenTemp,fridgeTemp,locale,units='metric',source='room',onSourceChange,
  ...
  idealWaterTemp,targetDoughTemp,directIceSupported=false,readOnly=false}:Props) {
  const fieldId=useId();
  ...
  return <div style={{marginTop:6}}>
    ...
    {clamped&&<div>{fr?'Cette eau seule ne permet pas d’atteindre la température de pâte visée.':'This water alone cannot reach the target dough temperature.'}</div>}

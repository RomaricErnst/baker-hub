...
function coldRequired(hours: number): number {
  return 50.2 / Math.pow(hours, 1.313);
}
...
//   progress_cold = IDY × h^1.313 / 50.2
...
function calcMultiStage(rtPhases: RTPhase[], coldHours: number): number {
  ...
  function contribution(idy: number): number {
    ...
    if (coldHours > 0) {
      total += idy * Math.pow(coldHours, 1.313) / 50.2;
    }
    return total;
  }
...
// T1: Neapolitan, 24h cold + 2h RT at 25°C → 0.05%–0.3%
const t1 = calcMultiStage([{ hours: 2, temp: 25 }], 24);
pass('T1 Neapolitan 24h cold + 2h RT @25°C', t1, 0.05, 0.3);
...
// T6: Very long, 72h cold at 4°C → at or near floor 0.05%
const t6 = calcCold(72);
passNear('T6 Very long 72h cold @4°C', t6, FLOOR, 0.05);

// T7: Multi-stage: 4h RT + 24h cold + 2h RT at 25°C → 0.05%–0.2%
const t7 = calcMultiStage([{ hours: 4, temp: 25 }, { hours: 2, temp: 25 }], 24);
pass('T7 Multi-stage 4h RT + 24h cold + 2h RT @25°C', t7, 0.05, 0.2);

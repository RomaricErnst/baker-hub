export type SandwichFamily = 'baguette' | 'focaccia' | 'bagel' | 'pita' | 'greek_pita' | 'kebab_bread' | 'batbout' | 'laffa' | 'piadina' | 'pan_bagnat' | 'ciabatta' | 'panuozzo';
...
// Recipe definitions follow below. Ingredient quantities are edible grams for ONE complete sandwich.
...
export function buildSandwichSteps(id: string, familyId: SandwichFamily, ingredients: SandwichPortion[], vegetarian: boolean): SandwichStep[] {
  ...
  if (['yogurt','tahini','lemon','mustard','honey','cumin','paprika','oregano'].some(has)) add('sauce','Préparer la sauce','Mix the sauce','Mélangez les ingrédients de sauce prévus : yaourt ou tahini, citron, moutarde, miel et épices selon la liste. Détendez le tahini avec un peu d’eau. N’ajoutez pas d’huile ou de mayonnaise en plus des quantités indiquées.','Mix the listed sauce ingredients: yogurt or tahini, lemon, mustard, honey and spices as applicable. Loosen tahini with a little water. Use only the listed amounts of oil or mayonnaise.',3,'prep');
